...
class GetUpcomingSessionsTest {
    ...
    fun setUp() {
        ...
        repository = mock()
        getUpcomingSessions = GetUpcomingSessions(repository, fixedClock)

    }

    @Test
    fun invoke_getUpcomingSessions() = runBlockingTest {
        val localSessions = listOf(
            Session("1", "s1", LocalDate.of(2020, 1, 1), emptyList(), SessionStatus.DONE),
        ...
        val upcomingSessions = getUpcomingSessions().first()
        assertThat(upcomingSessions).isEqualTo(localSessions.subList(2, 3))
    }
}